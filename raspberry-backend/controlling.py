...
class MQTTSubscriber:
    ...
    def __init__(self, sensor_id):
        ...
        self.client = mqtt.Client(client_id=sensor_id, protocol=mqtt.MQTTv311, transport="tcp")
        #self.client.tls_set(f"{ROOT_DIR}/{aws_iot['ca_path']}", certfile=f"{ROOT_DIR}/{aws_iot['cert_path']}", keyfile=f"{ROOT_DIR}/{aws_iot['key_path']}")
    ...
    def on_connect(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            ...
            print(control_topic, self.sensor_id)
    ...
    def start(self):
        """Connect to MQTT broker and handle automatic reconnection."""
        try:
            print("🔄 Connecting to MQTT broker...")
            self.client.connect(aws_iot["mqtt_broker"], aws_iot["mqtt_port"], keepalive=60)
            self.client.loop_forever()  # ✅ Run MQTT loop in the background
        except Exception as e:
...
if __name__ == "__main__":
    ...
    try:
        ...
        aws_iot = config["service_settings"]["aws_iot"]
